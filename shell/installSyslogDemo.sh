...
installDemoControl () {
		echo "*********************************Creating Demo Control service..."
       	...
       	curl -u admin:admin -H "X-Requested-By:ambari" -i -X POST http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/services/SYSLOG_DEMO_CONTROL

       	sleep 2
       	echo "*********************************Adding Demo Control component..."
       	# Add Demo Control component to service
       	curl -u admin:admin -H "X-Requested-By:ambari" -i -X POST http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/services/SYSLOG_DEMO_CONTROL/components/SYSLOG_DEMO_CONTROL
...
#/var/lib/ambari-server/resources/scripts/configs.sh set $AMBARI_HOST $CLUSTER_NAME control-config control-config
 ...
 #      	curl -u admin:admin -H "X-Requested-By:ambari" -i -X POST http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/hosts/$AMBARI_HOST/host_components/SYSLOG_DEMO_CONTROL

#       	sleep 15
 #      	echo "*********************************Installing Demo Control Service"
       	# Install Demo Control Service
  #     	TASKID=$(curl -u admin:admin -H "X-Requested-By:ambari" -i -X PUT -d '{"RequestInfo": {"context" :"Install Demo Control"}, "Body": {"ServiceInfo": {"maintenance_state" : "OFF", "state": "INSTALLED"}}}' http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/services/SYSLOG_DEMO_CONTROL | grep "id" | grep -Po '([0-9]+)')
...
#createPhoenixTables $ROOT_PATH/CloudBreakArtifacts/recipes/SYSLOG_DEMO_CONTROL/package/data
...
importPMMLModel $ROOT_PATH/CloudBreakArtifacts/recipes/SYSLOG_DEMO_CONTROL/package/model truck_demo_pmml.xml truck_demo_pmml 
echo "********************************* Import SAM Template"
TOPOLOGY_ID=$(importSAMTopology $ROOT_PATH/CloudBreakArtifacts/recipes/SYSLOG_DEMO_CONTROL/package/sam/SYSLOG-demo.json SYSLOG-Demo)
